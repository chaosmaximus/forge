...
HOOK_SCRIPT="$SCRIPT_DIR/../../scripts/post-edit-enhanced.sh"
FIXTURES="$SCRIPT_DIR/fixtures"

@test "exits 0 for nonexistent file" {
  run bash -c 'echo "{\"toolInput\":{\"file_path\":\"/tmp/nonexistent-forge-test-xyz.py\"}}" | bash '"'$HOOK_SCRIPT'"
  [ "$status" -eq 0 ]
}

@test "exits 0 for missing file_path" {
  run bash -c 'echo "{\"toolInput\":{}}" | bash '"'$HOOK_SCRIPT'"
  [ "$status" -eq 0 ]
}
...
@test "exits 0 for real python file" {
  ...
  echo 'x=1' > "$tmpfile"
  run bash -c "echo '{\"toolInput\":{\"file_path\":\"$tmpfile\"}}' | bash '$HOOK_SCRIPT'"
  [ "$status" -eq 0 ]
  rm -f "$tmpfile"
}
