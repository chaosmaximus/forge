...
- **Phase 17 (Protocol):** user-declared process rules. Current implementation at `crates/daemon/src/workers/consolidator.rs:1145-1226` queries memories where `memory_type = 'preference' OR (memory_type = 'pattern' AND title LIKE 'Behavioral:%')`, then in Rust further validates process-signal content ("always", "never", "must", "require", "workflow", "rule:") AND filters out observations ("validates", "proved that", "pipeline", etc.). Output = `memory(type='protocol')` rows. Represents "user says I should do X." Exact filter set subject to ongoing tuning; 2A-4c2 design must read the current code and pin the description there rather than restating this paraphrase.
...
9. Phase 23 is registered and executes after Phase 17 — verified via new test/bench-only `Request::ProbePhase { phase_name: "infer_skills_from_behavior" } -> { executed_at_phase_index: usize, executed_after: Vec<String> }` (added in 2A-4c2, gated under `#[cfg(any(test, feature = "bench"))]`). The `bench` Cargo feature declaration itself is owned by **whichever sub-phase ships the first feature-gated variant** (2A-4b's `ComputeRecencyFactor` or 2A-4c2's `ProbePhase`, whichever lands first) — see §13 "Resolve in 2A-4b OR 2A-4c2" bucket. Fallback: if the `bench` feature proves difficult to declare, both variants downgrade to `#[cfg(test)]` only and the bench harness accesses via integration-test style. Assertion checks `executed_at_phase_index > 17` — **where phase_index is 1-based to match the "Phase N" naming convention used in `consolidator.rs` comments and docs** (so Phase 17 has `executed_at_phase_index == 17`). Also checks `executed_after.contains("extract_protocols")` (actual Phase 17 function name at `crates/daemon/src/workers/consolidator.rs:1145`).
...
- **Bench timing sensitivity** — Dim 6a asserts `Request::ComputeRecencyFactor` output to ±0.0001 of the formula; if the formula parameters (e.g., `preference_half_life_days`) change, bench breaks. Mitigation: bench reads `preference_half_life_days` from config at startup and records it in `summary.json` alongside observed factors for regression traceability.
...
- **Phase 17 current behavior description correction (v6a):** the §5 2A-4c2 "Phase 17 (Protocol)" description has been updated to reflect actual code (SQL candidate selection on type + title prefix, followed by Rust-side process-signal validation and observation filtering). 2A-4c2 detailed design must re-verify against the current code at `consolidator.rs:1145-1226` and update the master if the filter set has changed since v6a.
