...
- `bench_name` (string, required): one of the canonical bench names —
  ...
  `forge-persist`, `forge-isolation`, `forge-coordination`,
  `longmemeval-<mode>`, `locomo-<mode>` (where `<mode>` is `raw` /
  `extract` etc., composed at the call site).
...
| `forge-coordination`    | 6 | `inbox_precision`, `roundtrip_correctness`, `broadcast_project_scoping`, `authorization_enforcement`, `edge_case_resilience`, `pipeline_chain_correctness` |
